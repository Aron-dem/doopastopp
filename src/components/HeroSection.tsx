      ...
      <main className="relative z-10 flex flex-col items-center justify-center flex-1 px-4 md:px-6 py-6 md:py-16 text-center">
        ...
        <div className="h-12 md:h-16 flex items-center justify-center mb-4 md:mb-6 fade-in-up"
          ...
          <p
            ...
            className="text-xl md:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-purple-600 max-w-2xl fade-in-up px-2"
          >
        ...
        <div className="flex flex-wrap justify-center gap-4 md:gap-8 mb-8 fade-in-up" style={{ animationDelay: '0.6s' }}>
          <div className="flex items-center gap-2 bg-white/5 px-4 py-2 rounded-full border border-white/10">
            ...
            <span className="text-xs text-gray-300">{t('+10,000 مستخدم', '+10,000 Users')}</span>
          </div>
          <div className="flex items-center gap-2 bg-white/5 px-4 py-2 rounded-full border border-white/10">
            <span className="text-purple-400">✓</span>
            <span className="text-xs text-gray-300">{t('أسس علمية', 'Scientific Basis')}</span>
          </div>
          <div className="flex items-center gap-2 bg-white/5 px-4 py-2 rounded-full border border-white/10">
            <span className="text-purple-400">🔒</span>
            <span className="text-xs text-gray-300">{t('خصوصية تامة', 'Full Privacy')}</span>
          </div>
        </div>
        ...
        <div className="w-full max-w-4xl mx-auto mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white/5 p-6 rounded-2xl border border-white/10 text-right">
              <p className="text-gray-300 italic mb-4">"{t("بعد استخدام خطة التعافي، تمكنت من قراءة كتاب كامل لأول مرة منذ سنوات. تركيزي تحسن بشكل ملحوظ!", "After using the recovery plan, I was able to read a full book for the first time in years. My focus has improved significantly!")}"</p>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-gradient-to-br from-purple-500 to-purple-800 rounded-full"></div>
                <div>
                  <h4 className="font-bold text-sm text-white">{t("أحمد م.", "Ahmed M.")}</h4>
                  <p className="text-xs text-gray-500">{t("طالب جامعي", "University Student")}</p>
                </div>
              </div>
            </div>
            <div className="bg-white/5 p-6 rounded-2xl border border-white/10 text-right">
              <p className="text-gray-300 italic mb-4">"{t("اختبار إدمان الهاتف كان جرس إنذار لي. التطبيق ساعدني في تقليل وقت الشاشة بـ 3 ساعات يومياً.", "The phone addiction test was a wake-up call for me. The app helped me reduce my screen time by 3 hours a day.")}"</p>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-gradient-to-br from-purple-600 to-purple-900 rounded-full"></div>
                <div>
                  <h4 className="font-bold text-sm text-white">{t("سارة ك.", "Sara K.")}</h4>
                  <p className="text-xs text-gray-500">{t("مصممة جرافيك", "Graphic Designer")}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Newsletter Signup */}
        <div className="w-full max-w-2xl mx-auto bg-gradient-to-r from-purple-900/40 to-purple-800/40 border border-white/10 p-8 md:p-12 rounded-[2rem] mb-12">
          <h2 className="text-2xl md:text-3xl font-black mb-4">{t("انضم إلى مجتمعنا", "Join Our Community")}</h2>
          <p className="text-gray-300 mb-8">{t("احصل على نصائح أسبوعية مجانية حول تحسين التركيز والإنتاجية مباشرة في بريدك الإلكتروني.", "Get free weekly tips on improving focus and productivity directly in your email.")}</p>
          <form className="flex flex-col sm:flex-row gap-3" onSubmit={(e) => e.preventDefault()}>
            <input 
              type="email" 
              placeholder={t("أدخل بريدك الإلكتروني...", "Enter your email...")} 
              className="flex-1 bg-white/10 border border-white/20 rounded-xl px-6 py-4 text-white focus:outline-none focus:border-purple-400 transition-colors"
              required
            />
            <button className="bg-purple-600 hover:bg-purple-500 text-white font-bold py-4 px-8 rounded-xl transition-colors whitespace-nowrap">
              {t("اشترك الآن", "Subscribe Now")}
            </button>
          </form>
          <p className="text-xs text-gray-500 mt-4">{t("نحترم خصوصيتك. لن نرسل لك رسائل مزعجة.", "We respect your privacy. No spam.")}</p>
        </div>
